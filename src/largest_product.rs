...
fn scanner(desired: usize, status: &mut (u64, usize), (d, m): (&u64, &u64)) -> Option<u64> {
    let (ref mut product, ref mut length) = *status;

    if *length == desired && *d != 0 {
        *product = *product / *d;
        *length = *length - 1;
    }
...
pub fn result(product_length: usize, data: &[u64]) -> u64 {
    iter::repeat(&1)
        .take(product_length)
        .chain(data.iter())
        .zip(data.iter())
        .scan((1, 0), |x, y| scanner(product_length, x, y))
        .max()
        .unwrap()
}
...
fn projecteuler() {
    ...
    assert_eq!(23_514_624_000, result(13, &input));
}
